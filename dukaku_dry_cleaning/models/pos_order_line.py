...
class PosOrderLine(models.Model):
    ...
    def _dry_cleaning_sync_garment_tags(self):
        ...
        if missing <= 0:
            return GarmentTag
        # Stage 6: create is no longer ACL-permitted for any group (tags
        # are infrastructure records generated by this controlled sync
        # method, never directly user-creatable) - sudo() is required here
        # for that reason alone. The only value passed in is self.id, an
        # existing pos.order.line this method already runs against (never
        # raw user-supplied data), so there is nothing here for an
        # elevated create() to launder. .with_env(self.env) immediately
        # after scopes the elevation to just this create() call.
        return GarmentTag.sudo().create([
